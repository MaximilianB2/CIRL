...
def PID_velocity(Ks, e, e_history, u_prev, ts, s_hist):
    ...
    KpCa = Ks[0]
    KiCa = Ks[1] + 1e-6
    KdCa = Ks[2]  # add epsilon to stop division by 0
    KpF = Ks[3]
    KiF = Ks[4] + 1e-6
    KdF = Ks[5]
    ...
    Tc = (
        u_prev[-1][0]
        + KpCa * (e[0] - e_history[-1, 0])
        + (KpCa / KiCa) * e[0] * dt
        - KpCa * KdCa * (e[0] - 2 * e_history[-1, 0] + e_history[-2, 0]) / dt
    )
    ...
    F = (
        u_prev[-1][1]
        + KpF * (e[1] - e_history[-1, 1])
        + (KpF / KiF) * e[1] * dt
        - KpF * KdF * (e[1] - 2 * e_history[-1, 1] + e_history[-2, 1]) / dt
    )
    F = min(max(F, 99), 105)
...
class reactor_class(gym.Env):
    ...
    def __init__(
        ...
        if self.highop:
            Ca_des1 = (
                [0.5 for i in range(int(ns / 3))]
                + [0.9 for i in range(int(ns / 3))]
                + [0.9 for i in range(int(ns / 3))]
            )

        else:
            Ca_des1 = (
                [0.70 for i in range(int(ns / 3))]
                + [0.75 for i in range(int(ns / 3))]
                + [0.86 for i in range(int(ns / 3))]
            )
        Ca_des2 = (
            [0.1 for i in range(int(ns / 3))]
            + [0.2 for i in range(int(ns / 3))]
            + [0.3 for i in range(int(ns / 3))]
        )
        Ca_des3 = (
            [0.4 for i in range(int(ns / 3))]
            + [0.5 for i in range(int(ns / 3))]
            + [0.6 for i in range(int(ns / 3))]
        )

        if self.test and self.highop:
            Ca_des1 = (
                [0.45 for i in range(int(ns / 3))]
                + [0.88 for i in range(int(ns / 3))]
                + [0.88 for i in range(int(ns / 3))]
            )
        elif self.test and not self.highop:
            Ca_des1 = (
                [0.075 for i in range(int(ns / 3))]
                + [0.45 for i in range(int(ns / 3))]
                + [0.725 for i in range(int(ns / 3))]
            )
        V_des = (
            [100 for i in range(int(ns / 3))]
            + [100 for i in range(int(ns / 3))]
            + [100 for i in range(int(ns / 3))]
        )
        ...
        )  # Cb,T,V,Cb,T,V,Ca_sp,V_sp
        if self.dist_obs:
            self.observation_space = spaces.Box(
                low=np.array([0, 350, 90, 0, 350, 90, 0, 99, 1]),
                high=np.array([1, 390, 102, 1, 390, 103, 1, 101, 2]),
            )  # Cb,T,V,Cb,T,V,Ca_sp,V_sp
        if self.normRL:
            self.action_space = spaces.Box(
                low=np.array([-1] * 2), high=np.array([1] * 2)
            )
        else:
            self.action_space = spaces.Box(
                low=np.array([-1] * 6), high=np.array([1] * 6)
            )
    ...
    def reset(self, seed=None):
        ...
        self.RL_state = [
            self.state[i]
            for i in [1, 3, 4, 6, 8, 9, 11, 13, 14, 15, 16, 17, 18, 19, 20]
        ]

        if self.dist_obs:
            self.RL_state.append(self.Caf)

        self.state_norm = (self.RL_state - self.observation_space.low) / (
    ...
    def reactor(self, state, action, Ca_des, V_des):
        ...
        if not self.DS and not self.normRL:
            try:
                Ks_norm = ((Ks.detach().numpy() + 1) / 2) * (
                    self.x_norm[1] - self.x_norm[0]
                ) + self.x_norm[0]
            ...
            if self.i < 2:
                ...
                u = PID_velocity(
                    Ks_norm,
                    e,
                    np.array(self.e_history),
                    self.u_history,
                    self.ts,
                    np.array(self.s_history),
                )
        if self.DS:
            u = self.u_DS
        ...
        T_plus = y[-1][3] + np.random.uniform(low=-0.1, high=0.1)
        V_plus = y[-1][4] + np.random.uniform(low=-0.01, high=0.01)
